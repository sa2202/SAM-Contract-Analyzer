...
# Ordered fallback list. Free-tier DAILY request quota is counted per model
# ("GenerateRequestsPerDayPerProjectPerModel"), so when one model is
# exhausted another still has a full allowance. Rotating automatically turns
# a hard stop into a transparent switch.
#
# This exists because the alternative is an administrator editing a Streamlit
# secret every time a quota runs out, for everyone, potentially daily. That
# is not a workable way to run a shared tool: it puts one person in the loop
# for every other person's failure, and it fails at exactly the moment
# someone is mid-task.
#
# Override with GEMINI_MODELS as a comma-separated list. GEMINI_MODEL (if
# set) always leads, so an explicit choice is still honoured first.
_FALLBACK_DEFAULT = "gemini-3.6-flash,gemini-3.7-flash,gemini-flash-lite-latest"


def _model_candidates() -> list[str]:
    """Ordered, de-duplicated list of models to try."""
    raw = os.getenv("GEMINI_MODELS", _FALLBACK_DEFAULT)
...
# Models whose DAILY quota is known to be spent. Process-level, so once one
# request discovers a model is exhausted, every later request in the same run
# skips it instead of rediscovering it the slow way -- one wasted call per
# contract across a batch adds up quickly against a 20/day budget.
#
# Keyed by (api_key_fingerprint, model) because quota is per project: one
# user's exhausted model says nothing about another user's key. The key
# itself is never stored, only a short hash of it.
_exhausted_models: set[tuple[str, str]] = set()


def _key_fingerprint(api_key: str | None) -> str:
    """Short, non-reversible tag for a key, so quota state can be tracked
    per project without holding the credential in memory."""
    import hashlib
    resolved = api_key or os.getenv(GEMINI_API_KEY_ENV) or "unset"
    return hashlib.sha256(resolved.encode()).hexdigest()[:12]


def _is_daily_quota_error(error_text: str) -> bool:
    """True for a per-DAY quota exhaustion, as opposed to a per-minute limit."""
    lowered = error_text.lower().replace(" ", "")
    return "perday" in lowered or "per_day" in lowered


def reset_exhausted_models():
    """Clear the exhausted-model cache (quotas reset at midnight Pacific)."""
...
# Free-tier ceilings for gemini-3.6-flash, read directly from Google AI
# Studio's own Rate Limit page (aistudio.google.com/rate-limit) rather than
# assumed -- third-party sources on this disagree with each other by two
# orders of magnitude and are not reliable. TPM 250,000 and RPM 5 were
# confirmed against a live account showing "65.8K / 250K" and "5 / 5".
#
# RPM matters more than it looks. A budget built around tokens alone judged
# only "how much text am I sending", and 5 requests/minute can be exhausted
# by a single contract long enough to chunk into 5 pieces, well before token
# usage becomes the binding constraint. TokenRateLimiter now tracks both.
#
# These are PER-PROJECT limits, i.e. per API key -- not shared across
# people using different keys. If you have several teammates hitting this
# limit, the fix is separate keys (see GEMINI_API_KEY handling below), not a
# bigger number here.
#
# Numbers drift as Google adjusts tiers; check
# https://aistudio.google.com/rate-limit against your own project if these
# stop matching what you see.
DEFAULT_TPM_BUDGET = 250000
DEFAULT_RPM_BUDGET = 5
...
def _get_client(api_key: str | None = None) -> OpenAI:
    ...
    `api_key`, when given, overrides the environment variable for this one
    call. This exists for the hosted deployment case: a Streamlit Cloud app
    has exactly one GEMINI_API_KEY in its secrets, so every visitor shares
    one project's free-tier quota -- 20 requests/day, 5/minute. That's
    fine for one person testing; it's gone within minutes once a team is
    using the same URL. Letting each person paste in their OWN free key
    (app.py's sidebar) means each teammate draws on their own quota instead
    of the deployment's shared one, at no cost to anyone.
    """
    resolved = api_key or os.getenv(GEMINI_API_KEY_ENV)
    if not resolved:
        raise ExtractionError(
            f"{GEMINI_API_KEY_ENV} is not set. Copy .env.example to .env and add "
            f"your key from {GEMINI_SIGNUP_HINT}."
        )
    return OpenAI(api_key=resolved, base_url=GEMINI_BASE_URL)
...
def _extract_with_model(
    ...
    """
    One extraction attempt against ONE named model.

    Callers should normally use extract_contract(), which wraps this with
    automatic fallback to another model when a daily quota is exhausted.
    """
    ...
    for attempt in range(1, max_retries + 2):  # e.g. max_retries=1 -> try twice total
        try:
            ...
            if "model_not_found" in error_str or "does not exist" in error_str or "404" in error_str:
                ...
                # Gemini returns 429 for two very different problems, and
                # telling them apart decides what the user should actually do.
                # A PER-MINUTE limit clears in under a minute. A PER-DAY quota
                # does not clear until midnight Pacific -- telling someone to
                # "try again in a minute" when they have hit the daily cap
                # sends them to retry, fail, and retry again with no idea why.
                # Google's payload names the metric, so read it rather than
                # guessing: the daily one contains "PerDay".
                is_daily = "perday" in error_str.replace(" ", "") or "per_day" in error_str

                if is_daily:
                    last_error = ExtractionError(
                        "Daily quota used up on Gemini's free tier (20 requests "
                        "per day, per model). This does NOT reset in a minute -- "
                        "it resets at midnight US Pacific time (about 12:30 PM "
                        "IST).\n\n"
                        "Options right now:\n"
                        "1. Enter your own free API key in the sidebar -- each key "
                        "has its own separate quota "
                        "(https://aistudio.google.com/apikey).\n"
                        "2. Switch GEMINI_MODEL to a different model: the quota is "
                        "counted per model, so another one gives you a fresh "
                        "allowance.\n"
                        "3. Enable billing on the Google Cloud project to remove "
                        "the daily cap entirely.\n\n"
                        f"(Original error: {e})"
                    )
                else:
                    last_error = ExtractionError(
                        f"Per-minute rate limit hit on Gemini (free tier allows 5 "
                        f"requests/minute). Wait about a minute and try again -- a "
                        f"long contract split into several sections can hit this on "
                        f"its own. (Original error: {e})"
                    )
            else:
...
def extract_contract(
    ...
    """
    Extract a contract, automatically falling back to another model if the
    current one's DAILY quota is exhausted.

    Free-tier daily request quota is counted per model, so an exhausted model
    doesn't mean an exhausted key -- there is usually a full allowance sitting
    on the next model in the list. Rotating here means a user mid-task sees a
    brief switch instead of a hard stop, and nobody has to edit a deployment
    secret on their behalf.

    Only DAILY quota errors trigger a switch. A per-minute limit is handled by
    the rate limiter (waiting clears it, and switching models would waste the
    other model's daily allowance to dodge a 60-second wait). Any other error
    is a real failure and is raised as-is rather than retried across every
    model in turn, which would multiply one bad document into several failures.

    `on_model_switch(from_model, to_model)` is an optional callback so the UI
    can mention the switch.
    """
    fingerprint = _key_fingerprint(api_key)

    candidates = _model_candidates() if model is None else [model]
    available = [m for m in candidates if (fingerprint, m) not in _exhausted_models]

    # Everything already known-exhausted: try the original order anyway rather
    # than failing without an attempt. Quotas reset at midnight Pacific and
    # this cache has no clock, so a stale entry must never be the sole reason
    # a request isn't made.
    if not available:
    ...
    for index, candidate in enumerate(available):
        try:
            return _extract_with_model(
                document_text, document_filename,
                model=candidate, max_retries=max_retries, api_key=api_key,
            )
        except ExtractionError as e:
            last_error = e
            if not _is_daily_quota_error(str(e)):
                raise  # not a quota problem -- another model won't help
            _exhausted_models.add((fingerprint, candidate))
            if index + 1 < len(available):
                if on_model_switch:
                    on_model_switch(candidate, available[index + 1])
                continue
            raise

    raise last_error or ExtractionError("No model available to try.")


def _run_reduce_summary(
    merged_extraction: ContractExtraction,
    model: str,
    limiter: TokenRateLimiter,
    api_key: str | None = None,
) -> str:
    # Note: the caller treats a failure here as non-fatal and keeps the
    # concatenated per-chunk summary, so this deliberately does NOT rotate
    # models. Spending another model's scarce daily allowance to improve a
    # summary -- when a usable one already exists -- is the wrong trade.
    """
...
def extract_contract_chunked(
    ...
    rpm_budget: int = DEFAULT_RPM_BUDGET,
    ...
    `api_key` overrides GEMINI_API_KEY for every call this makes -- lets a
    caller (see batch.py, app.py) use a per-user key instead of the one in
    the environment, so a hosted deployment shared by a team doesn't force
    everyone onto one project's free-tier quota.
    ...
    if limiter is None:
        limiter = TokenRateLimiter(tpm_budget=tpm_budget, rpm_budget=rpm_budget)
    ...
    for chunk in chunks:
        ...
        result = extract_contract(
            chunk.text, document_filename, model=model, api_key=api_key,
    ...
    if merge_result.chunk_count > 1:
        ...
        try:
            merge_result.merged.plain_english_summary = _run_reduce_summary(
                merge_result.merged, model or DEFAULT_MODEL, limiter, api_key=api_key
            )
        except Exception:
