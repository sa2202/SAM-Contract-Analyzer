...
def extract_contract(
    ...
    `api_key` overrides GEMINI_API_KEY for this call -- see _get_client.
...
def extract_contract_chunked(
    ...
    for chunk in chunks:
        ...
        result = extract_contract(chunk.text, document_filename, model=model, api_key=api_key)
    ...
    if merge_result.chunk_count > 1:
        ...
        try:
            merge_result.merged.plain_english_summary = _run_reduce_summary(
                merge_result.merged, model, limiter, api_key=api_key
            )
        except Exception:
