...
from comparison import (
    build_comparison, clause_comparison, comparison_headline,
)
...
st.set_page_config(
    ...
    page_icon="📑",
    layout="wide",
    # Expanded, not collapsed. The sidebar holds the personal API key field,
    # which is the one thing a new user on a shared deployment needs to find
    # BEFORE their first run -- without it they silently spend the
    # deployment's shared free-tier quota and hit a rate limit they have no
    # way to diagnose. A collapsed sidebar hides that behind a small arrow
    # most people never click, which makes the feature effectively invisible
    # to exactly the users it exists for.
...
/* Comparison table */
.cmp-worst {
    color: #E06C65; font-weight: 600;
}
.cmp-absent-risk {
    color: #E06C65; font-style: italic;
}
.cmp-headline {
    font-size: 0.84rem; color: #B8C4BF;
    border-left: 2px solid rgba(74,147,130,0.5);
    padding: 0.15rem 0 0.15rem 0.7rem; margin: 0.1rem 0 0.9rem 0;
}
.cmp-note { font-size: 0.76rem; color: #7C8A85; margin-bottom: 0.5rem; }
...
# ---------------------------------------------------------------------------
# Personal API key (sidebar)
# ---------------------------------------------------------------------------
# When this app is deployed centrally (Streamlit Cloud), everyone who opens
# the URL shares whichever key is in the app's own secrets -- one project's
# free-tier quota (20 requests/day, 5/minute on gemini-3.6-flash at time of
# writing) split across the whole team. That's fine for one person testing;
# it's gone within minutes once several people use the same link.
#
# Letting each person paste in their OWN free key means each teammate draws
# on their own quota instead. Nobody needs to pay anything -- everyone can
# get a free key at https://aistudio.google.com/apikey. This field is
# entirely optional: leave it blank and the app falls back to whatever key
# the deployment has configured, which is exactly today's behaviour.
with st.sidebar:
    ...
    # Which key is in use is stated plainly either way. The failure this
    # prevents is quiet: without your own key you share one free-tier quota
    # with everyone else using this link, and the resulting rate limit
    # surfaces as an opaque error mid-analysis rather than as a hint to set
    # a key.
    if st.session_state.user_api_key:
        st.success("Using your own key — your quota is separate.", icon="✓")
    else:
        st.warning(
            "Using this app's shared key. Everyone on this link shares one "
            "free daily quota, so it runs out quickly.",
            icon="⚠️",
        )
        st.markdown(
            "**[Get a free key →](https://aistudio.google.com/apikey)**  \n"
            "Free, no card needed. Paste it above and it's used for this "
            "session only — it isn't stored or shared."
        )
    ...
    with st.status(f"Analyzing {total} file(s)…", expanded=True) as status:
        try:
            with SessionWorkspace() as ws:
                ...
                def on_model_switch(from_model, to_model):
                    # Stated rather than silent. A user who sees results
                    # arrive from a different model than they configured
                    # should know why -- and it's the signal that the day's
                    # quota on the first model is gone.
                    st.write(
                        f"　↻ Daily quota used up on `{from_model}` — "
                        f"switching to `{to_model}` and carrying on."
                    )

                batch = analyze_batch(
                    paths, on_file_start=on_start, on_file_done=on_done,
...
# ---------------------------------------------------------------------------
    ...
    # "Compare" only appears with 2+ contracts -- comparing one contract
    # against itself answers nothing, and an empty tab invites a click that
    # leads nowhere. Tab indices are resolved by name below rather than
    # hardcoded, so the optional tabs can't silently shift the others.
    tab_names = ["Entitlement summary", "Clause finder"]
    if len(successful) > 1:
        tab_names.append("Compare")
    tab_names += ["Risks", "Contract detail"]
    if batch.failed:
    ...
    # --- Compare (side-by-side across contracts) ---------------------------
    # The clause finder stacks clauses so they can be READ. This answers the
    # question a reviewer asks immediately afterwards -- "which of these is
    # worst?" -- which stacked prose makes you work out by scrolling and
    # holding five numbers in your head.
    if "Compare" in tab:
        with tab["Compare"]:
            vendors = [r.vendor for r in successful]

            mode = st.radio(
                "Compare",
                ["Key terms", "Clause text"],
                horizontal=True,
                label_visibility="collapsed",
            )

            if mode == "Key terms":
                rows = build_comparison(successful)
                if not rows:
                    st.info("No comparable terms were extracted from these contracts.")
                else:
                    st.markdown(
                        '<div class="note">Red marks the least favourable contract for '
                        "that term. Only terms found in at least one contract are shown, "
                        "and a term is only ranked when two or more contracts have a "
                        "comparable value.</div>",
                        unsafe_allow_html=True,
                    )

                    table = {"Term": [r.label for r in rows]}
                    for col, vendor in enumerate(vendors):
                        table[vendor] = [r.cells[col].display for r in rows]
                    ...
                    def highlight(row):
                        ...
                        for i in range(len(vendors)):
                            if i == worst:
                                styles.append("color: #E06C65; font-weight: 600")
                            elif i in absent:
                                styles.append("color: #E06C65; font-style: italic")
                            else:
                                styles.append("")
                        return styles
                    ...
                    st.dataframe(
                        ...
                        height=min(80 + 36 * len(rows), 620),
                    )

                    st.markdown("##### What stands out")
                    said_something = False
                    for spec in rows:
                        headline = comparison_headline(spec, successful)
                        if not headline:
                            continue
                        said_something = True
                        st.markdown(
                            f"**{h(spec.label)}** &nbsp;"
                            f'<span class="cmp-headline">{h(headline)}</span>',
                            unsafe_allow_html=True,
                        )
                        if spec.note:
                            st.markdown(
                                f'<div class="cmp-note">{h(spec.note)}</div>',
                                unsafe_allow_html=True,
                            )
                    if not said_something:
                        st.caption(
                            "Nothing to rank — no term had a comparable value in two or "
                            "more contracts."
                        )

            else:
                # Clause text side by side, in columns, so wording differences
                # are visible without scrolling between them.
                covered = [
                    ct for ct in CLAUSE_TYPES
                    if any(c.clause_type == ct
                           for r in successful for c in r.extraction.key_clauses)
                ]
                if not covered:
                    st.info("No clauses were captured from these contracts.")
                else:
                    choice = st.selectbox("Clause type", covered)
                    entries = clause_comparison(successful, choice)
                    found = sum(1 for e in entries if e["found"])
                    st.caption(f"{found} of {len(entries)} contracts contain this clause.")

                    columns = st.columns(min(len(entries), 3))
                    for i, entry in enumerate(entries):
                        with columns[i % len(columns)]:
                            page = f" · p.{entry['page']}" if entry["page"] else ""
                            if not entry["found"]:
                                st.markdown(
                                    f'<div class="clause none">'
                                    f'<div class="clause-who">{h(entry["vendor"])}</div>'
                                    f'<div class="clause-absent">Not found</div></div>',
                                    unsafe_allow_html=True,
                                )
                                continue
                            body = (f'<div class="clause-body">{h(entry["text"])}</div>'
                                    if entry["text"] else "")
                            plain = (f'<div class="clause-plain">{h(entry["summary"])}</div>'
                                     if entry["summary"] else "")
                            st.markdown(
                                f'<div class="clause">'
                                f'<div class="clause-who">{h(entry["vendor"])}{page}</div>'
                                f'<div class="clause-head">'
                                f'{h(entry["heading"] or choice)}</div>'
                                f"{body}{plain}</div>",
                                unsafe_allow_html=True,
                            )
    ...
    # --- Unreadable files ---------------------------------------------------
    if batch.failed:
        with tab[f"Not read ({len(batch.failed)})"]:
            st.warning(
